...
def stream_id_spliter(stream_id_len, filesize):
    if filesize <= 500:
        ...
        if stream_id_len >= 500: n_split = 20
        if stream_id_len > 5000: n_split = 50 + stream_id_len*0.1
    if filesize >= 500:
        if stream_id_len < 50: n_split = 20 + stream_id_len*0.1
        if stream_id_len >= 50: n_split = 40
        if stream_id_len >= 500: n_split = 50 + stream_id_len*0.2
        if stream_id_len > 5000: n_split = 200 + stream_id_len*0.3
...
def worker_udp_extractor(file_path, start_stream_id, end_stream_id):
    ...
    if IS_GQUIC is True: data = 'gguic.payload'
    data = 'data'
    cmd = f'''tshark -r {file_path} \
...
if __name__ == '__main__':
    ...
    for file_path in ls_subfolders(PCAP_LOC):
        ...
        tcp_stream_split, udp_stream_split = stream_id_spliter(tcp_stream_len, filesize), stream_id_spliter(udp_stream_len, filesize)
        ...
        # Filter udp.stream
        tasks = []
        # pd.DataFrame(columns=[['time_epoch'] +]).to_csv(f'./{TEMP_PART_LOC}/udp_{filename}.csv', index=False)
