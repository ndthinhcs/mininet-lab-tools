...
def ls_protocol(file):
    result, _ = subprocess.Popen(f'tshark  -r {file} -qz io,phs',
...
def stream_id_len(filename, saved_filename):
    ...
    result_tcp = result_tcp.decode('utf-8').split('\n')[start_index_report-1:-2]
    result_udp = result_udp.decode('utf-8').split('\n')[start_index_report-1:-2]
    
    with open(f"{saved_filename}_summary.csv", "w") as txt_file:
        txt_file.write('l4_proto,stream_id,A->B,B->A,total\n')
        id = 0
        for arr in result_tcp:
            arr = [x for x in arr.split(' ') if x != '']
            line = f'tcp,{id},{arr[3]},{arr[5]},{arr[7]}'
            id+=1
            txt_file.write(f'{line} \n')
        id = 0
        for arr in result_udp:
            arr = [x for x in arr.split(' ')  if x != '']
            line = f'udp,{id},{arr[3]},{arr[5]},{arr[7]}'
            id+=1
            txt_file.write(f'{line} \n')
...
if __name__ == '__main__':
    ...
    for file_path in ls_subfolders(PCAP_LOC):
        ...
        # Merge csv
        result, err = subprocess.Popen(f'echo "time_epoch,frame_number,stream_id,ip_src,ip_dst,ip_proto,protocol,length,info,data" > {saved_filename}.csv', 
                        stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True).communicate()
        print(result, err)
